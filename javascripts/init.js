...
dnd_cc.buildTable = function () {
  ...
  Object.keys(dnd_cc.state.inventory).forEach(function (key) {
    if ($("tr." + key.replace(/\ /g,'-' )).length > 0) {
      dnd_cc.increaseItem(name);
    }
    var itemRow = $("<tr class='" + key.replace(/\ /g,'-' ) + "'>");
...
$('body').on('click', 'button.addOne', function (event) {
  ...
  dnd_cc.increaseItem(this.parentElement.parentElement.classList[0].replace(/\-/, " "));
});

$('body').on('click', 'button.removeOne', function (event) {
  console.log("this", this);
  console.log("event", event);
  dnd_cc.decreaseItem(this.parentElement.parentElement.classList[0].replace(/\-/, " "));
});
